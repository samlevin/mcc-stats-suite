...
t.test(
  'dataset splits are stable and depend only on screenshot identity and seed',
  (t) => {
    const first = splitForScreenshot('screenshot-1', 'seed');
    t.equal(splitForScreenshot('screenshot-1', 'seed'), first);
    t.equal(
      splitForScreenshot('screenshot-1', 'seed'),
      first,
      'replayed runs cannot cross partitions',
    );
...
t.test(
  'dataset splitter uses all three partitions over a representative population',
  (t) => {
...
t.test(
  'materializer joins cells, tokens, validation, versions, and source provenance',
  (t) => {
